...
export default function MessagesAdmin() {
  ...
  const fetchMessages = useCallback(async () => {
    try {
      ...
      if (filters.isReplied !== undefined) queryParams.append('isReplied', filters.isReplied.toString());
  ...
  const deleteMessage = async () => {
    ...
    try {
      setDeleting(true);
      const response = await fetch(`/api/messages/${selectedMessage.id}/delete?permanent=${deletePermanent}`, {
        method: 'DELETE',
      });
                        ...
                        <div>
                          <p className="text-sm text-gray-700">
                            ...
                            <span className="font-medium">
                              {Math.min(filters.page * filters.limit, messages.length + (filters.page - 1) * filters.limit)}
                            </span>{' '}
                            sur <span className="font-medium">{messages.length + (totalPages - filters.page) * filters.limit}</span> résultats
                          </p>
                        </div>
                        <div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto m-auto">
            <div className="flex justify-between items-center p-6 border-b">
              <h2 className="text-2xl font-bold text-gray-800">Répondre à {selectedMessage.fullName}</h2>
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full m-auto">
            <div className="p-6">
              ...
              <p className="text-gray-600 mb-4">
                Êtes-vous sûr de vouloir supprimer le message de <strong>{selectedMessage.fullName}</strong> ?
              </p>
              ...
              <label className="flex items-center gap-2 mb-4">
                ...
                />
                <span className="text-sm text-gray-700">Supprimer définitivement (ne pourra pas être restauré)</span>
              </label>
